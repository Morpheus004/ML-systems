...
## Project Context & Mission: CUDA SGEMM Optimization

### Objective
Progressively optimize matrix multiplication (SGEMM) kernels from scratch toward cuBLAS-level performance on an NVIDIA GeForce RTX 4060 Laptop GPU (Ada Lovelace, `sm_89`), following Simon Boehm's worklog: [*How to Optimize a CUDA Matmul Kernel for cuBLAS-like Performance*](https://siboehm.com/articles/22/CUDA-MMM).

### Working Style
The student writes the code. The agent provides conceptual guidance, reviews logic, explains hardware/architecture nuances (warps, memory hierarchy, coalescing, shared memory), and suggests debugging checks.

### Optimization Roadmap
1. **Naive SGEMM**: Baseline 1 thread per element, uncoalesced global memory access.
2. **Global Memory Coalescing**: Sequential thread memory access matching warp structure.
3. **Shared Memory Cache Blocking**: Staging tiles in `__shared__` memory to reduce global memory traffic.
4. **1D Block Tiling**: Multiple output elements per thread along one dimension.
5. **2D Block Tiling**: Register tiling ($TM \times TN$) with outer products in registers.
6. **Vectorized Memory Access**: 128-bit wide memory loads (`float4` / `LDG.E.128`).
7. **Bank Conflict Resolution**: Eliminating shared memory bank conflicts.
8. **Double Buffering**: Overlapping global memory load latency with arithmetic.
9. **Autotuning**: Tuning tile parameters for target GPU SMs.
